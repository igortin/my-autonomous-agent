...
#### Goal  
 ...
 Цель перестаёт быть просто фразой в истории сообщений и становится явным машинно-читаемым контрактом AgentGoal. Преобразуем свободный запрос юзера в проверяемый контракт.

Для запроса: Intent - Исходное намерение. Понятно человеку, но недостаточно формально для control loop.
```
Восстанови payment-api до 3 реплик
```

Нормализованное желаемое состояние. Граф должен получить примерно такой объект:
```
{
